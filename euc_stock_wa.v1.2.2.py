...
# New save_plots function
def save_plots():
    """
    Saves an image of each inventory data to a user-specified folder, headlessly.
    """
    # Prompt the user to select a folder
    save_dir = filedialog.askdirectory(title="Select a folder to save the plots")
    if not save_dir:
        tk.messagebox.showinfo("Info", "No folder selected. Operation cancelled.")
        return

    # Define the scripts and their corresponding output filenames
    inventory_scripts = [
        ("inventory-levels_4.2v2.py", "Basement_4.2_inventory.png"),
        ("inventory-levels_BRv2.2.py", "Build_Room_inventory.png"),
        ("inventory-levels_darwinv1.py", "Darwin_inventory.png"),
        ("inventory-levels_combinedv1.4.py", "Combined_inventory.png"),
    ]
    ...
    for script_name, output_file in inventory_scripts:
        ...
        if script_path.exists():
            try:
                ...
                os.system(f"python {script_path} {output_path}")  # Pass output path as an argument
            except Exception as e:
    ...
    tk.messagebox.showinfo("Info", f"All plots have been saved in {save_dir}.")
...
def view_san_returns_log():
    ...
    return_asset_button = ctk.CTkButton(
        ...
        text="Return Asset",
        font=("Helvetica", 14),
        command=open_san_return_form  # Opens the SAN Return Form
    )
    return_asset_button.pack(pady=10)  # Positioned at the bottom of the log window


def submit_san_return(san, returned_by, returned_to, notes, timestamp, form_window, returns_tree):
    """
    Validates input and submits the data to the SAN_Returns sheet.
    Updates the Treeview dynamically after submission.
    """
    # Check if required fields are filled
    if not san or not returned_by or not returned_to or not timestamp:
    ...
    if 'SAN_Returns' not in workbook.sheetnames:
        san_returns_sheet = workbook.create_sheet('SAN_Returns')
        san_returns_sheet.append(["SAN", "Returned By", "Returned To", "Notes", "Timestamp"])  # Add headers
    else:
        san_returns_sheet = workbook['SAN_Returns']

    # Append the data to the SAN_Returns sheet
    san_returns_sheet.append([san, returned_by, returned_to, notes, timestamp])
...
#     command=open_san_return_form  # Opens the SAN Return Form
# )
# return_asset_button.pack(pady=10)  # Adjust positioning as needed
